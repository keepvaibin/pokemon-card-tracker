...
export default function CardDetailModal({
  ...
  const makeDateTicks = (min: number, max: number, maxTicks = 5) => {
    ...
    for (let i = 0; i < ticks.length; i++) {
      ...
      if (i === 0 || i === ticks.length - 1 || lab !== lastLabel) {
        out.push(ticks[i]);
        lastLabel = lab;
      }
    }
    ...
    // trim to <= 5
    while (out.length > 5) {
      // remove every second inner tick
  ...
  // ---- modern, hoverable dropdown: no gap between button and menu
  function SeriesDropdown({
    ...
    const scheduleClose = () => {
      ...
      closeTimer.current = window.setTimeout(() => setOpen(false), 150); // small hover-intent delay
    };
    ...
    return (
      ...
      >
        ...
        {/* Hover guard bridging the gap so leaving the trigger doesn't instantly close the menu */}
        {open && (
  ...
  // ---- line chart: robust reveal via pathLength (no clip-path flakiness)
  function LineChart({
    pts, xMin, xMax, dark,
    ...
    const xToPx = (t: number) => leftPad + ((t - xMin) / Math.max(1, xMax - xMin)) * innerW;
    const yToPx = (v: number) => topPad + (1 - (v - yMin) / Math.max(1e-9, yMax - yMin)) * innerH;

    const d = pts.map((p, i) => `${i ? 'L' : 'M'} ${xToPx(p.x.getTime()).toFixed(2)} ${yToPx(p.y).toFixed(2)}`).join(' ');
    const areaPath = pts.length > 1
      ? `${d} L ${xToPx(pts[pts.length-1].x.getTime())} ${yToPx(0)} L ${xToPx(pts[0].x.getTime())} ${yToPx(0)} Z`
      : '';

    // ticks (kept same behavior)
    const makeDateTicks = (min: number, max: number, maxTicks = 5) => {
      if (min >= max) return [min, max];
      const want = Math.max(2, Math.min(maxTicks, 5));
      const ticks: number[] = [];
      for (let i = 0; i < want; i++) ticks.push(min + Math.round((i / (want - 1)) * (max - min)));
      const fmt = (t: number) => new Date(t).toISOString().slice(0,10);
      const out: number[] = []; let last: string | null = null;
      for (let i = 0; i < ticks.length; i++) { const lab = fmt(ticks[i]); if (i === 0 || i === ticks.length - 1 || lab !== last) { out.push(ticks[i]); last = lab; } }
      if (out[0] !== min) out.unshift(min);
      if (out[out.length - 1] !== max) out.push(max);
    ...
    // ---- MotionValues + imperative animate() (no controls.start warnings)
    const pathLen = useMotionValue(0);
    const clipW = useMotionValue(0);

    useEffect(() => {
      const stop1 = animate(pathLen, 1, { duration: 2.0, ease: [0.4, 0, 0.2, 1] });
      const stop2 = animate(clipW, innerW, { duration: 2.0, ease: [0.4, 0, 0.2, 1] });
      return () => { stop1.stop(); stop2.stop(); };
      // mount only – plays once per mount (switching series remounts LineChart intentionally)
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    // keep revealed area width in sync on resize without re-animating
    useEffect(() => {
      clipW.set(innerW);
    }, [innerW, clipW]);
    ...
    return (
      <div ref={wrapRef} className="w-full">
        <svg width="100%" height={H} role="img" onMouseMove={onMove} onMouseLeave={onLeave}>
          ...
          {pts.length > 1 && (
            <>
              <path d={areaPath} fill={`url(#${uid}-grad)`} clipPath={`url(#${uid}-clip)`} opacity={0.9} />
              <motion.path
                d={d}
                initial={false}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto relative">
          ...
          <div className={`px-6 pt-5 pb-3 bg-gradient-to-b ${theme.headerBg} border-b ${theme.border}`}>
            ...
            {/* Chart card (extra top padding so dropdown never overlaps) */}
            <div
            ...
            >
              ...
              {points.length > 0 ? (
                // key ensures clean animation when the selected series changes
          ...
          {/* Body (extra bottom padding so footer never covers content) */}
